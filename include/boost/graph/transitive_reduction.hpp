...
#include <EASTL/algorithm.h> //eastl::find
    ...
    VertexIndexMap g_index_map)
{
    ...
    typedef typename eastl::vector< Vertex >::size_type size_type;

    eastl::vector< Vertex > topo_order;
    topological_sort(g, eastl::back_inserter(topo_order));

    eastl::vector< size_type > topo_number_storage(num_vertices(g));

    iterator_property_map< size_type*, VertexIndexMap, size_type, size_type& >
        topo_number(&topo_number_storage[0], g_index_map);

    {
        typename eastl::vector< Vertex >::reverse_iterator it
            = topo_order.rbegin();
        size_type n = 0;
    ...
    eastl::vector< eastl::vector< bool > > edge_in_closure(
        num_vertices(g), eastl::vector< bool >(num_vertices(g), false));
    {
        typename eastl::vector< Vertex >::reverse_iterator it
            = topo_order.rbegin();
    ...
    typename eastl::vector< Vertex >::iterator it = topo_order.begin(),
                                             end = topo_order.end();
    for (; it != end; ++it)
    {
        ...
        // do that. So what I'm doint is, collection the successors of *it here
        {
            ...
            for (eastl::tie(oi, oi_end) = out_edges(*it, g); oi != oi_end; ++oi)
            {
...

        {
            ...
            typename eastl::vector< Vertex >::reverse_iterator rit
                = topo_order.rbegin(),
                rend = topo_order.rend();
            for (; rit != rend; ++rit)
            {
                // looking if they are successors of *it
                if (eastl::find(neighbors.begin(), neighbors.end(), *rit)
                    != neighbors.end())
                {
