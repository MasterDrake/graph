...
/*
  ...
  eastl::vector as the "back-bone" of the graph (the vector *is* the
...
template < class EdgeList > struct graph_traits< eastl::vector< EdgeList > >
{
    ...
    typedef typename eastl::vector< EdgeList >::size_type vertices_size_type;
    typedef void edges_size_type;
    typedef typename EdgeList::size_type degree_size_type;
    static V null_vertex() { return V(-1); }
};
template < class EdgeList > struct edge_property_type< eastl::vector< EdgeList > >
{
    typedef void type;
};
template < class EdgeList >
struct vertex_property_type< eastl::vector< EdgeList > >
{
    typedef void type;
};
template < class EdgeList >
struct graph_property_type< eastl::vector< EdgeList > >
{
    typedef void type;
};
}
...
namespace detail
{
    ...
    template < class V, class Iter > class val_out_edge_iterator
    {
        ...
        typedef eastl::pair< V, V > Edge;

    public:
        typedef eastl::input_iterator_tag iterator_category;
        typedef eastl::pair< V, V > value_type;
        typedef std::ptrdiff_t difference_type;
        typedef eastl::pair< V, V >* pointer;
        typedef const eastl::pair< V, V > reference;
    ...
    template < class EdgeList > struct val_out_edge_ret
    {
        ...
        typedef eastl::pair< IncIter, IncIter > type;
    };
...
typename detail::val_out_edge_ret< EdgeList >::type out_edges(
    typename EdgeList::value_type v, const eastl::vector< EdgeList, Alloc >& g)
{
...
template < class EdgeList, class Alloc >
eastl::pair< typename EdgeList::const_iterator,
    typename EdgeList::const_iterator >
adjacent_vertices(
    typename EdgeList::value_type v, const eastl::vector< EdgeList, Alloc >& g)
{
    return eastl::make_pair(g[v].begin(), g[v].end());
}

// source() and target() already provided for pairs in graph_traits.hpp

template < class EdgeList, class Alloc >
eastl::pair< boost::counting_iterator< typename EdgeList::value_type >,
    boost::counting_iterator< typename EdgeList::value_type > >
vertices(const eastl::vector< EdgeList, Alloc >& v)
{
    typedef boost::counting_iterator< typename EdgeList::value_type > Iter;
    return eastl::make_pair(Iter(0), Iter(v.size()));
}

template < class EdgeList, class Alloc >
typename eastl::vector< EdgeList, Alloc >::size_type num_vertices(
    const eastl::vector< EdgeList, Alloc >& v)
{
    return v.size();
}

template < class EdgeList, class Allocator >
typename eastl::pair< typename detail::val_edge< EdgeList >::type, bool >
add_edge(typename EdgeList::value_type u, typename EdgeList::value_type v,
    eastl::vector< EdgeList, Allocator >& g)
{
    typedef typename detail::val_edge< EdgeList >::type edge_type;
    g[u].insert(g[u].end(), v);
    return eastl::make_pair(edge_type(u, v), true);
}

template < class EdgeList, class Allocator >
typename eastl::pair< typename detail::val_edge< EdgeList >::type, bool > edge(
    ...
    eastl::vector< EdgeList, Allocator >& g)
{
    ...
    for (; i != end; ++i)
        if (*i == v)
            return eastl::make_pair(edge_type(u, v), true);
    return eastl::make_pair(edge_type(), false);
}

template < class EdgeList, class Allocator >
void remove_edge(typename EdgeList::value_type u,
    typename EdgeList::value_type v, eastl::vector< EdgeList, Allocator >& g)
{
    typename EdgeList::iterator i = eastl::remove(g[u].begin(), g[u].end(), v);
    if (i != g[u].end())
        g[u].erase(i, g[u].end());
}
...
void remove_edge_if(Predicate p, eastl::vector< EdgeList, Allocator >& g)
{
...
template < class EdgeList, class Allocator >
typename EdgeList::value_type add_vertex(eastl::vector< EdgeList, Allocator >& g)
{
    g.emplace_back();
    return g.size() - 1;
}
...
template < class EdgeList, class Allocator >
void clear_vertex(
    typename EdgeList::value_type u, eastl::vector< EdgeList, Allocator >& g)
{
    g[u].clear();
...
struct property_map< eastl::vector< EdgeList, Allocator >, vertex_index_t >
{
...
identity_property_map get(
    vertex_index_t, const eastl::vector< EdgeList, Allocator >&)
{
    return identity_property_map();
}

template < typename EdgeList, typename Allocator >
identity_property_map get(vertex_index_t, eastl::vector< EdgeList, Allocator >&)
{
    return identity_property_map();
}
} // namespace boost
