...
template < typename UndirectedGraph > void undirected_graph_demo1()
{
    ...
    for (eastl::tie(out, out_end) = out_edges(zero, undigraph); out != out_end;
         ++out)
        std::cout << ' ' << *out;
    std::cout << std::endl << "in_edges(0):";
    for (eastl::tie(in, in_end) = in_edges(zero, undigraph); in != in_end; ++in)
        std::cout << ' ' << *in;
    std::cout << std::endl;
}
...
template < typename DirectedGraph > void directed_graph_demo()
{
    ...
    eastl::tie(e1, found) = edge(u, v, digraph);
    eastl::tie(e2, found) = edge(v, u, digraph);
...
template < typename UndirectedGraph > void undirected_graph_demo2()
{
    ...
    eastl::tie(e1, found) = edge(u, v, undigraph);
    eastl::tie(e2, found) = edge(v, u, undigraph);
    ...
    for (eastl::tie(e, e_end) = out_edges(s, undigraph); e != e_end; ++e)
